...
double retard(int DragFunction, double DragCoefficient, double Velocity){
	...
	switch(DragFunction) {
		case G0:
			if (vp> 2600) { A = 1.5366e-03; M = 1.67;}
			else if (vp> 2000) { A = 5.8497e-03; M = 1.5;}
			else if (vp> 1460) { A = 5.9814e-04; M = 1.8;}
			else if (vp> 1190) { A = 9.5408e-08; M = 3.0;}
			else if (vp> 1040) { A = 2.3385e-18; M = 6.45;}
			else if (vp> 840)  { A = 5.9939e-08; M = 3.0;}
			else if (vp> 0)    { A = 7.4422e-04; M = 1.6;}
			break;
...
int SolveAll(int DragFunction, double DragCoefficient, double Vi, double SightHeight, double ShootingAngle, double ZAngle, double WindSpeed, double WindAngle, double** Solution){
